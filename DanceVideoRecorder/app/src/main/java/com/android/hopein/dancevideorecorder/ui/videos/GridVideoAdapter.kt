...
class GridVideoAdapter(var context: Context, var list: MutableList<CommonUtils.Video>): BaseAdapter() {
    ...
    override fun getView(position: Int, view: View?, viewGroup: ViewGroup?): View {
        var view = view
        if(view == null){
            view = layoutInflater.inflate(R.layout.item_video_content, viewGroup, false)
        }

        val image = view?.findViewById<ImageView>(R.id.mediaImage)
        val duration = view?.findViewById<TextView>(R.id.mediaDuration)
        ...
        duration?.text = list[position].duration
